<template>
  <article class="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-md text-center">
    <div class="relative aspect-[3/4] overflow-hidden bg-secondary">
      ...
      <span v-if="producto.en_oferta" class="absolute left-2.5 top-2.5 flex items-center gap-1 rounded-full bg-primary px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-primary-foreground">
        <Tag class="h-3 w-3" /> Oferta
      </span>
      <span class="absolute right-2.5 top-2.5 rounded-full bg-card/90 px-2 py-0.5 text-[10px] font-medium text-muted-foreground backdrop-blur-sm">
        #{{ producto.id }}
      </span>
      <span class="absolute bottom-2.5 right-2.5 rounded-full bg-card/90 px-2 py-0.5 text-[12px] font-bold text-primary backdrop-blur-sm border border-primary">
...
const imgSrc = computed(() => props.producto && props.producto.imagen ? buildImageUrl(props.producto.imagen) : '')
</script>
