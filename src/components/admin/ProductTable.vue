    ...
    <table class="min-w-full text-left text-sm">
      ...
      <tbody>
        <tr v-for="p in productos" :key="p.id" class="border-t border-border hover:bg-secondary/40">
          ...
          <td class="px-4 py-2">
            <img v-if="p.imagen" :src="buildImageUrl(p.imagen)" alt="img" class="w-12 h-12 object-cover rounded border" />
          ...
          <td class="px-4 py-2">
            <span v-if="p.en_oferta" class="rounded-full bg-primary text-primary-foreground px-2 py-1 text-xs font-bold">Sí</span>
            <span v-else class="text-muted-foreground text-xs">No</span>
          </td>
          <td class="px-4 py-2">
            <span v-if="p.para_mujer" class="rounded-full bg-pink-100 text-pink-700 px-2 py-1 text-xs font-bold">Mujer</span>
            <span v-else class="rounded-full bg-blue-100 text-blue-700 px-2 py-1 text-xs font-bold">Hombre</span>
          </td>
          <td class="px-4 py-2 flex gap-2">
            <button @click="$emit('edit', p)" class="icon-btn bg-primary/90 hover:bg-primary" title="Editar">
              <Pencil :size="18" />
            </button>
            <button @click="$emit('delete', p)" class="icon-btn bg-destructive/90 hover:bg-destructive" title="Eliminar">
              <Trash2 :size="18" />
            </button>
          </td>
        </tr>
