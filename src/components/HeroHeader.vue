  ...
  <header class="relative overflow-hidden bg-secondary text-center">
    ...
    <div class="relative z-10 mx-auto max-w-4xl px-6 py-5 flex flex-col items-center">
      <nav class="flex flex-col items-center gap-5 text-center w-full">
        ...
        <div
          class="flex flex-wrap items-center justify-center gap-1 rounded-full border border-border bg-card/80 p-1.5 text-sm backdrop-blur-sm"
        >
          <a
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary cursor-pointer"
            @click.prevent="$emit('navFilter', 'todos')"
            >Catálogo</a
          >
          <a
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary cursor-pointer"
            @click.prevent="$emit('navFilter', 'ofertas')"
            >Ofertas</a
          >
          <a
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary cursor-pointer"
            @click.prevent="$emit('navFilter', 'hombre')"
            >Hombre</a
          >
          <a
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary cursor-pointer"
            @click.prevent="$emit('navFilter', 'mujer')"
            >Mujer</a
          >
          <a
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary cursor-pointer"
            @click.prevent="$emit('navFilter', 'recientes')"
            >Lo último</a
          >
          <a
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary cursor-pointer"
            @click.prevent="$emit('navFilter', 'baratos')"
            >Lo más económico</a
          >
          <button
            class="rounded-full px-4 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-primary"
            type="button"
      ...
      <div class="mt-12 w-full flex flex-col items-center justify-center">
        <div class="flex flex-col gap-5 items-center w-full">
          ...
          <p class="max-w-lg text-base text-muted-foreground leading-relaxed text-center">
          ...
          <div class="flex flex-wrap gap-3 pt-1 justify-center w-full">
            ...
            <a
              class="catalogo-btn-fix flex items-center justify-center rounded-full px-6 py-3 text-sm font-semibold uppercase tracking-widest mx-auto"
              href="#catalogo"
              >Ver catálogo</a
            >
          </div>
